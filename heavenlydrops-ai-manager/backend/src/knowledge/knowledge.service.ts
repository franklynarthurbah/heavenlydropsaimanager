...
export class KnowledgeService {
  ...
  private async syncSource(url: string, documentType: DocumentType): Promise<boolean> {
    ...
    if (mainContent.length) {
      content = this.extractTextWithStructure(mainContent, $);
    } else {
   ...
   */
  private extractTextWithStructure(element: cheerio.Cheerio, $: cheerio.Root): string {
    const sections: string[] = [];
    
    element.find('h1, h2, h3, h4, h5, h6, p, li').each((_, el) => {
