...
import { WhatsAppClassifierService } from '../integrations/whatsapp-classifier.service';
import { AiService } from '../ai/ai.service';
import { LeadsService } from '../leads/leads.service';
import { Lead, LeadSource, InterestType } from '../leads/entities/lead.entity';
...
export class ConversationsService {
  ...
  constructor(
    ...
    private whatsappClassifier: WhatsAppClassifierService,
  ...
  async handleIncomingMessage(message: IncomingMessage): Promise<{
    ...
    // ── Classify message intent (WhatsApp / Instagram) ──────────────────────
    if (
      message.channel === ConversationChannel.WHATSAPP ||
      message.channel === ConversationChannel.INSTAGRAM
    ) {
      try {
        const recentHistory = await this.messageRepository.find({
          where: { conversationId: conversation.id },
          order: { createdAt: 'DESC' },
          take: 5,
        });
        const historyTexts = recentHistory.reverse().map(m => m.content);

        const classification = await this.whatsappClassifier.classifyMessage(
          message.content,
          historyTexts,
        );

        // Update lead interestType if classifier is more confident
        if (
          classification.confidence >= 0.7 &&
          classification.interestType !== InterestType.OTHER &&
          lead.interestType === InterestType.OTHER
        ) {
          await this.leadsService.update(lead.id, {
            interestType: classification.interestType,
          } as any);
          lead.interestType = classification.interestType;
        }

        // Attach classification to conversation context
        conversation.context = {
          ...conversation.context,
          topic: classification.intent,
          qualificationScore: Math.round(classification.confidence * 100),
          nextAction:
            classification.urgency === 'high'
              ? 'Urgent — assign to human agent immediately'
        ...
        // If urgency is HIGH, flag for human attention
        if (classification.urgency === 'high') {
          conversation.requiresHumanAttention = true;
          await this.conversationRepository.save(conversation);
        }
      } catch (classifyErr) {
        this.logger.warn('Classification failed, continuing without it:', classifyErr.message);
      }
    }
